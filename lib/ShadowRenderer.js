...
export default function ShadowRenderer(position, height) {

	const PLANE_WIDTH = 20;
	const PLANE_HEIGHT = 20;
	const CAMERA_HEIGHT = height || 2;
	...
	function init() {
		...
		shadowGroup.position.y = position.y;
		...
		const planeGeometry = new THREE.PlaneGeometry(PLANE_WIDTH + Math.random() * 2, PLANE_HEIGHT + Math.random() * 2).rotateX( Math.PI / 2 );
		const planeMaterial = new THREE.MeshBasicMaterial( {
			color: state.plane.color, // Constants.bgColor,
	...
	function blurShadow(renderer, amount) {
